function Tank(x, y, src, arag , type){
...
Tank.prototype.draw = function(canvas){
	...
	let xx = images[this.src][0];
	let yy = images[this.src][1];
	
	ctx.drawImage(img,32 * this.dir + xx, yy, 32, 32,this.x + offerX,this.y + offerY,32,32) ;	
...
function clearTank(x,y){
	...
	let ctx = myCanvas.getContext("2d");
	let imgTank = document.getElementById("Tank");
	
	
	ctx.clearRect(x + offerX,y + offerY,32,32);
}
...
function updataTanks(){
	var i;
	var map_sada = new Array(25);

	for(i=0; i<25; i++)
	map_sada[i] = new Array(25);

	p_maps(map, map_sada);

	for(i = playerNum; i < tanks.length ; i ++)
	{
		if(stopTime == 0) tanks[i].moveFree(map_sada);		
		tanks[i].updata();
	}
