...
BombFx.prototype.draw = function(cv){
	let Cvs = document.getElementById(cv);
	let ctx = Cvs.getContext("2d");
	let img = document.getElementById("spr");
	
	ctx.drawImage(img,this.frame * 67 + images[this.src][0], images[this.src][1], 66, 66, this.x + offerX, this.y + offerY, 66, 66) ;
	return;
};
...
function drawBombFxs(){
	...
	for(let i = 0;i < bombFxs.length;i++)
	{
		bombFxs[i].draw("main");
	}
}
